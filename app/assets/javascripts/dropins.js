$(document).on('turbolinks:load', () => {
  ...
  $(".view-dropin").tooltip({    container: "body"  });
  $(".edit-dropin").tooltip({    container: "body"  });
  $(".delete-dropin").tooltip({    container: "body"  });
  $(".selectpicker").selectpicker();
  $("#pay-now-button").click(function () {    $(this).button("loading");  });
  $('.email-dropin-creator').on('click', function () {    $('#dropin_creator_emails_modal').modal('show');  });
  $(".invite-skaters").on("click", function () {    $("#invite_modal").modal("show");  });
  $('.email-attendees').on('click', function () {    $('#email_attendees_modal').modal('show')  });
...
//        message.val(message.val() + $('#dropin-terms-and-conditions').text());
  ...
  description.on('change', function () {
    return updateCountdown('#dropin_description');
  });
